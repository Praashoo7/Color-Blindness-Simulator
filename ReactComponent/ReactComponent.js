import React, { useState } from 'react';

const ColorBlindnessFilter = () => {
  const [currentFilterIndex, setCurrentFilterIndex] = useState(0);

  const filters = [
    { name: 'None type [0]', value: 'none' },
    { name: 'Protanopia [1]', value: 'url(#protanopia)' },
    { name: 'Protanomaly [2]', value: 'url(#protanomaly)' },
    { name: 'Deuteranopia [3]', value: 'url(#deuteranopia)' },
    { name: 'Deuteranomaly [4]', value: 'url(#deuteranomaly)' },
    { name: 'Tritanopia [5]', value: 'url(#tritanopia)' },
    { name: 'Tritanomaly [6]', value: 'url(#tritanomaly)' },
    { name: 'Achromatopsia [7]', value: 'url(#achromatopsia)' },
    { name: 'Achromatomaly [8]', value: 'url(#achromatomaly)' }
  ];

  const applyColorBlindnessFilter = () => {
    const filter = filters[currentFilterIndex].value;
    document.body.style.filter = filter;
  };

  const updateSpanInfo = () => {
    const filterValue = filters[currentFilterIndex].value;
    document.body.style.filter = filterValue;
    const filterName = filters[currentFilterIndex].name;
    return filterName;
  };

  const handleColorBlindnessClick = () => {
    setCurrentFilterIndex((currentFilterIndex + 1) % filters.length);
    applyColorBlindnessFilter();
  };

  return (
    <div className="mainColorBlindness">
      <button id="colorBlindnessButton" onClick={handleColorBlindnessClick}>
        {/* SVG content */}
        <svg class="eye_main" width="24" height="24" viewBox="0 0 30 30" version="1.1" xmlnsXlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg">
        <defs>
            <path d="M0,15.089434 C0,16.3335929 5.13666091,24.1788679 14.9348958,24.1788679 C24.7325019,24.1788679 29.8697917,16.3335929 29.8697917,15.089434 C29.8697917,13.8456167 24.7325019,6 14.9348958,6 C5.13666091,6 0,13.8456167 0,15.089434 Z" id="outline" fill="black"></path>
            <mask id="mask">
            <rect width="100%" height="100%" fill="white"></rect>
            <use xlinkHref="#outline" id="lid" fill="black"/>
            </mask>
        </defs>
        <g id="eye">
            <path id="eye_eye" d="M0,15.089434 C0,16.3335929 5.13666091,24.1788679 14.9348958,24.1788679 C24.7325019,24.1788679 29.8697917,16.3335929 29.8697917,15.089434 C29.8697917,13.8456167 24.7325019,6 14.9348958,6 C5.13666091,6 0,13.8456167 0,15.089434 Z M14.9348958,22.081464 C11.2690863,22.081464 8.29688487,18.9510766 8.29688487,15.089434 C8.29688487,11.2277914 11.2690863,8.09740397 14.9348958,8.09740397 C18.6007053,8.09740397 21.5725924,11.2277914 21.5725924,15.089434 C21.5725924,18.9510766 18.6007053,22.081464 14.9348958,22.081464 L14.9348958,22.081464 Z M18.2535869,15.089434 C18.2535869,17.0200844 16.7673289,18.5857907 14.9348958,18.5857907 C13.1018339,18.5857907 11.6162048,17.0200844 11.6162048,15.089434 C11.6162048,13.1587835 13.1018339,11.593419 14.9348958,11.593419 C15.9253152,11.593419 14.3271242,14.3639878 14.9348958,15.089434 C15.451486,15.7055336 18.2535869,14.2027016 18.2535869,15.089434 L18.2535869,15.089434 Z" ></path>
            <use xlinkHref="#outline" mask="url(#mask)" fill="#FFFFFF"/>
        </g>
    </svg>
      </button>
      <span id="filterName">
        Type : <br />
        {updateSpanInfo()}
        <span style={{ marginLeft: '0.25em' }} id="filterName">
        </span>
      </span>

      {/* SVG Filters */}
      <svg class="COLOR-FILTERS-SVG"
      xmlns="http://www.w3.org/2000/svg"
      version="1.1">
      <defs>
          <filter id="protanopia">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.567, 0.433, 0,     0, 0
                      0.558, 0.442, 0,     0, 0
                      0,     0.242, 0.758, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
          <filter id="protanomaly">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.817, 0.183, 0,     0, 0
                      0.333, 0.667, 0,     0, 0
                      0,     0.125, 0.875, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
          <filter id="deuteranopia">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.625, 0.375, 0,   0, 0
                      0.7,   0.3,   0,   0, 0
                      0,     0.3,   0.7, 0, 0
                      0,     0,     0,   1, 0"/>
          </filter>
          <filter id="deuteranomaly">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.8,   0.2,   0,     0, 0
                      0.258, 0.742, 0,     0, 0
                      0,     0.142, 0.858, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
          <filter id="tritanopia">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.95, 0.05,  0,     0, 0
                      0,    0.433, 0.567, 0, 0
                      0,    0.475, 0.525, 0, 0
                      0,    0,     0,     1, 0"/>
          </filter>
          <filter id="tritanomaly">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.967, 0.033, 0,     0, 0
                      0,     0.733, 0.267, 0, 0
                      0,     0.183, 0.817, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
          <filter id="achromatopsia">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.299, 0.587, 0.114, 0, 0
                      0.299, 0.587, 0.114, 0, 0
                      0.299, 0.587, 0.114, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
          <filter id="achromatomaly">
          <feColorMatrix
              in="SourceGraphic"
              type="matrix"
              values="0.618, 0.320, 0.062, 0, 0
                      0.163, 0.775, 0.062, 0, 0
                      0.163, 0.320, 0.516, 0, 0
                      0,     0,     0,     1, 0"/>
          </filter>
      </defs>
      </svg>

      {/* CSS Styles */}
      <style>
        {`
          @import url('https://fonts.googleapis.com/css2?family=Josefin+Slab:ital,wght@0,100..700;1,100..700&display=swap');
    
          .COLOR-FILTERS-SVG{
              display: none;
          }
          
          .mainColorBlindness{
            position: relative;
            padding: 0.5em;
            padding-right: 0.85em;
            border-radius: 10px;
            display: flex;
            flex-direction: row;
            align-items: center;
            justify-content: start;
            gap: 0.75em;
            border: 1px solid black;
            background-color: #F8EBC9;
            z-index: 99999;
            width: max-content;
            height: max-content;
          }
      
          #filterName, #filterValue{
            font-family: Josefin Slab;
            color: black;
            line-height: 1.15em;
            font-weight: bold;
          }
          #filterName{
            width: 139px;
          }
      
          #colorBlindnessButton{
              background-color: transparent;
              border: 1px solid black;
              border-radius: 8px;
              width: 3em;
              height: 3em;
              display: flex;
              align-items: center;
              justify-content: center;
              cursor: pointer;
              transition: .4s ease-in-out;
          }
          #colorBlindnessButton:hover{
              background-color: #171717;
          }
          #eye_eye{
              fill: black;
              transition: .4s ease-in-out;
          }
          #colorBlindnessButton:hover #eye_eye{
              fill: white;
          }
      
          #lid{
            animation: eye 2s ease-in-out infinite;
          }
          #colorBlindnessButton:hover #lid{
            animation: none;
          }
        
          @keyframes eye {
            0%{
                transform: scaleX(1) scaleY(1);
                transform-origin: center;
            }
            10%{
                transform: scaleX(1) scaleY(0);
                transform-origin: center;
            }
            20%{
                transform: scaleX(1) scaleY(1);
                transform-origin: center;
            }
            75%{
                transform: scaleX(1) scaleY(1);
                transform-origin: center;
            }
            80%{
                transform: scaleX(1) scaleY(0);
                transform-origin: center;
            }
            85%{
                transform: scaleX(1) scaleY(1);
                transform-origin: center;
            }
            100%{
                transform: scaleX(1) scaleY(1);
                transform-origin: center;
            }
          }
        `}
      </style>
    </div>
  );
};

export default ColorBlindnessFilter;